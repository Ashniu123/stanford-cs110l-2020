...
impl OpenFile {
    ...
    pub fn from_fd(pid: usize, fd: usize) -> Option<OpenFile> {
        let pb = fs::read_link(format!("/proc/{}/fd/{}", pid, fd)).ok()?;
        let name = OpenFile::path_to_name(pb.to_str()?);
        let pb = fs::read_to_string(format!("/proc/{}/fdinfo/{}", pid, fd)).ok()?;
        let cursor = OpenFile::parse_cursor(&pb)?;
        let access_mode = OpenFile::parse_access_mode(&pb)?;

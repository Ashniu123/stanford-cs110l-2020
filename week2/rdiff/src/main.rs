...
fn lcs(seq1: &Vec<String>, seq2: &Vec<String>) -> Grid {
    ...
    for i in 0..seq1.len() + 1 {
        for j in 0..seq2.len() + 1 {
            if i == 0 || j == 0 {
                assert!(grid.set(i, j, 0).is_ok());
            } else if seq1[i - 1] == seq2[j - 1] {
                assert!(grid.set(i, j, grid.get(i - 1, j - 1).unwrap() + 1).is_ok());
            } else {
                assert!(grid
                    .set(
                        i,
...
fn main() {
    ...
    let lines1 = read_file_lines(filename1).expect("Failed to read");
    let lines2 = read_file_lines(filename2).expect("Failed to read");
